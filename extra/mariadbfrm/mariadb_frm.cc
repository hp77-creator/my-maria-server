...
static int init_plugin_system_complete()
{
  ...
  my_rnd_init(&sql_rand, (ulong) server_start_time,
              (ulong) server_start_time / 2);
...
static bool parse_frm_file(THD *fake_thd, const char *frm_path)
{
  ...
  share= (TABLE_SHARE *) my_malloc(PSI_NOT_INSTRUMENTED, sizeof(TABLE_SHARE),
                                   MYF(MY_ZEROFILL));
  if (!share)
  {
  ...
  char *norm_path=
      (char *) alloc_root(&share->mem_root, strlen(path_buff) + 1);
  if (norm_path)
  {
  ...
  int parse_error= share->init_from_binary_frm_image((THD *) fake_thd, false,
                                                     frm_data, frm_length);
  if (parse_error != 0)
  {
